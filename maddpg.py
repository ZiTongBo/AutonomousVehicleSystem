...
class MADDPG(object):
    def __init__(self, a_dim, s_dim, a_bound, model, retrain):
        ...
        self.a2 = tf.placeholder(tf.float32, [None, a_dim], 'a')
        self.a_2 = tf.placeholder(tf.float32, [None, a_dim], 'a22')
    ...
    def learn(self):
        ...
        bs_1 = bt[:, -self.s_dim * 2 - self.a_dim: -self.s_dim - self.a_dim]
        bs_2 = bt[:, -self.s_dim - self.a_dim:-self.a_dim]
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ...
            net = tf.layers.dense(s, 30, activation=tf.nn.tanh, kernel_initializer=init_w,
                                  bias_initializer=init_b, name='l11', trainable=trainable)
            # a = tf.layers.dense(net, self.a_dim, name='a', trainable=trainable)
            # a1 = tf.layers.dense(net, 60, activation=tf.nn.tanh, kernel_initializer=init_w,
            #                      bias_initializer=init_b, name='a11', trainable=trainable)
            a2 = tf.layers.dense(net, self.a_dim, activation=tf.nn.tanh, kernel_initializer=init_w,
                                 bias_initializer=init_b, name='a21', trainable=trainable)
            return a2

    def _build_c(self, s1, s2, a, oa, scope, trainable):
        with tf.variable_scope(scope):
            n_l1 = 30
            n_l2 = 60
            w1_s = tf.get_variable('w1_s1', [self.s_dim, n_l1], trainable=trainable)
            w2_s = tf.get_variable('w2_s2', [self.s_dim, n_l1], trainable=trainable)
            w1_a = tf.get_variable('w1_a1', [self.a_dim, n_l1], trainable=trainable)
            w1_oa = tf.get_variable('w1_oa1', [self.a_dim, n_l1], trainable=trainable)
            b = tf.get_variable('b', [1, n_l1], trainable=trainable)
            net1 = tf.nn.tanh(tf.matmul(s1, w1_s) + tf.matmul(s2, w2_s) + tf.matmul(a, w1_a) + tf.matmul(oa, w1_oa) + b)
